...
const modelManager = {
  ...
  updateCenterModelUI: function () {
    ...
    if (centerModelSelect) {
      ...
      if (
        previousCenterValue &&
        this.availableModels.includes(previousCenterValue)
      ) {
...
document.addEventListener("keydown", function (e) {
  ...
  if (isAlphaNumeric && notInInput && noModifiers) {
    ...
    // This code is the worst. There should be a way to avoid this
...
function initEventListeners() {
  ...
  document
    ...
    .addEventListener("click", showModelSelection);
  ...
  document
    .getElementById("importChatInput")
    .addEventListener("change", importChat);
...
function setupDragAndDrop() {
  ...
  dragArea.innerHTML =
    '<div class="drag-area-content"><i class="fas fa-cloud-upload-alt"></i><p>Drop images here</p></div>';
...
function handleFiles(files) {
  if (files.length > 0) {
    ...
    tempPreviewContainer.style.cssText =
      "margin: 10px 0; padding: 10px; border-radius: 8px; background-color: #f8f9fa; max-width: 100%;";
    ...
    infoText.style.cssText =
      "margin-top: 5px; font-size: 14px; color: #6c757d;";
    ...
    imageFiles.forEach((file) => {
      ...
      reader.onload = function (e) {
        ...
        img.onload = function () {
          ...
          console.log(
            "Image processed, total attachments:",
...
function loadChatHistory() {
  try {
    ...
    Object.keys(loadedChats).forEach((chatId) => {
      if (
        loadedChats[chatId].messages &&
        loadedChats[chatId].messages.length > 0
      ) {
  ...
  if (chats[chatId]) {
    ...
    modelManager.fetchModels(() => {
      ...
      if (chats[chatId].modelName) {
        modelIsAvailable = modelManager.isModelAvailable(
          chats[chatId].modelName,
        );
        if (modelIsAvailable) {
      ...
      if (!chats[chatId].messages || chats[chatId].messages.length === 0) {
        ...
        disableInputWithMessage(
          `The model "${chats[chatId].modelName}" is currently unavailable. You can start a new chat.`,
        );
      } else {
    ...
    if (cleanMessage) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (line.trim() || i < lines.length - 1) {
          // Display empty lines except last one
  ...
  reader.onload = function (e) {
    try {
      ...
      if (modelName && !modelManager.isModelAvailable(modelName)) {
        if (
          !confirm(
            `The model "${modelName}" may not be available. Import anyway?`,
          )
        ) {
          return;
        }
      ...
      if (!modelAvailable) {
        disableInputWithMessage(
          `The model "${modelName}" is currently unavailable. You can start a new chat.`,
        );
      }
...
function handleSendMessage() {
  ...
  if (!modelManager.isModelAvailable(selectedModel)) {
    alert(
      `The model "${selectedModel}" is currently unavailable. Please select another model.`,
    );
    return;
  }
